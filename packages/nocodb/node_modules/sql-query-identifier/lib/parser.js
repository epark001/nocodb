...
function nextNonWhitespaceToken(state) {
    let token;
    do {
        state = initState({ prevState: state });
        token = (0, tokenizer_1.scanToken)(state);
...
function createStatementParserByToken(token, nextToken, options) {
...
function createShowStatementParser(options) {
    const statement = createInitialStatement();
    const steps = [
        {
            preCanGoToNext: () => false,
        ...
        // Database/Table/Columns/...
        {
            ...
            add: (token) => {
                statement.type = `SHOW_${token.value.toUpperCase().replace(' ', '_')}`;
            },
            postCanGoToNext: () => true,
        },
    ];
    ...
    return {
        ...
        addToken(token, nextToken) {
            ...
            if (token.type === 'keyword' &&
                ...
                (prevNonWhitespaceToken === null || prevNonWhitespaceToken === void 0 ? void 0 : prevNonWhitespaceToken.value.toUpperCase()) !== 'END' &&
            ...
            if (['psql', 'mssql', 'bigquery'].includes(dialect) &&
            ...
            if (dialect !== 'sqlite' &&
                (token.value.toUpperCase() === 'OR' ||
                    ((prevNonWhitespaceToken === null || prevNonWhitespaceToken === void 0 ? void 0 : prevNonWhitespaceToken.value.toUpperCase()) === 'OR' &&
            ...
            if (statement.definer !== undefined && statement.definer > 0) {
                if (statement.definer === 1 && (prevToken === null || prevToken === void 0 ? void 0 : prevToken.type) === 'whitespace') {
                    statement.definer++;
                    setPrevToken(token);
                    return;
                }
            ...
            if (statement.algorithm !== undefined && statement.algorithm > 0) {
                if (statement.algorithm === 1 && (prevToken === null || prevToken === void 0 ? void 0 : prevToken.type) === 'whitespace') {
                    statement.algorithm++;
                    setPrevToken(token);
                    return;
                }
